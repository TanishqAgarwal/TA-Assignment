...
router.post("/createtask", async(req, res) => {
    ...
    try{
        const newTask = await TaskModel.create({name, description, dueDate:ISODate, period, periodType});
...
router.get("/tasklist", async(req, res) => {
    ...
    if(!searchText)
    {
      ...
      try{
        ...
        if(tasksPerPage && pageNumber){
             tasks = await TaskModel.find({}).skip(tasksPerPage*(pageNumber-1)).limit(tasksPerPage);
        }
        else{
    ...
    else
    {
        ...
        const nameMatchingTask = await TaskModel.findOne({name : {$regex : searchText,$options:"$i"}});
        const descriptionMatchingTask = await TaskModel.findOne({description : {$regex : searchText,$options:"$i"}});
        let tasks =[];
